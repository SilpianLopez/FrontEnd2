...
public class RoutineMainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 1) Intent extras 먼저 읽기
        int fromIntent = getIntent().getIntExtra(KEY_USER_ID, -1);
        if (fromIntent != -1) {
            currentUserId = fromIntent;
            Log.d(TAG, "Intent로 받은 user_id = " + currentUserId);
        } else {
            // 2) SharedPreferences에서 읽기
            SharedPreferences prefs = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
            currentUserId = prefs.getInt(KEY_USER_ID, -1);
            Log.d(TAG, "Prefs에서 읽어온 user_id = " + currentUserId);
        }

        // 3) 로그인 필요 시 로그인 화면으로 이동
        if (currentUserId == -1) {
        ...
        Log.d(TAG, "최종 user_id = " + currentUserId);
